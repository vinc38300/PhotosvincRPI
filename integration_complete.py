...
Max: {profile.get('max_width', 'N/A')}x{profile.get('max_height', 'N/A')}mm
    ...
    def show_zip_cleanup_settings(self):
        """Affiche les paramètres de nettoyage des ZIP"""
        settings_win = tk.Toplevel(self.root)
        settings_win.title("Paramètres ZIP")
        settings_win.geometry("500x400")
        settings_win.configure(bg='#2c3e50')
        ...
        settings_win.geometry(f"500x400+{x}+{y}")
        
        tk.Label(
            settings_win,
            text="⚙️ GESTION DES ZIP",
            font=('Arial', 18, 'bold'),
            bg='#2c3e50',
            fg='#ecf0f1'
        ).pack(pady=20)
        
        info_frame = tk.Frame(settings_win, bg='#34495e', relief=tk.RIDGE, bd=2)
        info_frame.pack(fill=tk.X, padx=30, pady=15)
        
        current_lifetime = self.gallery_downloader.zip_lifetime_minutes
        
        tk.Label(
            info_frame,
            text=f"Durée de vie actuelle: {current_lifetime} minutes",
            font=('Arial', 13, 'bold'),
            bg='#34495e',
            fg='#3498db'
        ).pack(pady=10)
        ...
        tk.Label(
            info_frame,
            text="Les archives ZIP sont automatiquement\nsupprimées après expiration",
            font=('Arial', 11),
            bg='#34495e',
            fg='#ecf0f1',
            justify=tk.CENTER
        ...
        def set_lifetime(minutes):
            self.gallery_downloader.zip_lifetime_minutes = minutes
            messagebox.showinfo(
                "Paramètre modifié",
                f"Durée de vie des ZIP: {minutes} minutes\n\n"
                f"Les ZIP plus anciens seront supprimés\nlors du prochain nettoyage.",
                parent=settings_win
            )
            settings_win.destroy()
        
        for minutes, label in durations:
            bg_color = '#27ae60' if minutes == current_lifetime else '#34495e'
            tk.Button(
                settings_win,
                text=label,
                font=('Arial', 11, 'bold'),
                ...
                width=25,
                height=2,
                command=lambda m=minutes: set_lifetime(m)
            ).pack(pady=5)
        ...
        tk.Button(
            ...
            width=25,
            height=2,
            command=lambda: self.manual_zip_cleanup(settings_win)
        ).pack(pady=15)
    ...
    def manual_zip_cleanup(self, parent_win=None):
        """Nettoyage manuel des ZIP expirés"""
        count = self.gallery_downloader.cleanup_expired_zips()
        
        if count > 0:
            messagebox.showinfo(
                "Nettoyage effectué",
                f"🧹 {count} archive(s) expirée(s) supprimée(s)",
                parent=parent_win
            )
        else:
            messagebox.showinfo(
                "Aucun nettoyage",
                "Aucune archive expirée à supprimer",
                parent=parent_win
            )

    def start_zip_cleanup_timer(self):
        """Démarre le nettoyage périodique des ZIP expirés"""
        def cleanup_task():
            try:
                self.gallery_downloader.cleanup_expired_zips()
            except Exception as e:
                print(f"Erreur nettoyage ZIP: {e}")
            
            # Répéter toutes les 10 minutes (600000 ms)
            self.root.after(600000, cleanup_task)
        
        # Premier nettoyage après 10 minutes
        self.root.after(600000, cleanup_task)

    def download_gallery(self):
        """Télécharge toutes les photos en ZIP avec option QR code"""
        stats = self.gallery_downloader.get_gallery_stats()
        ...
        # Centrer
        progress_win.update_idletasks()
        x = (progress_win.winfo_screenwidth() // 2) - 225
        y = (progress_win.winfo_screenheight() // 2) - 125
        progress_win.geometry(f"450x250+{x}+{y}")
        ...
        info_frame = tk.Frame(progress_win, bg='#34495e', relief=tk.RIDGE, bd=2)
        info_frame.pack(fill=tk.X, padx=30, pady=10)
        
        tk.Label(
            info_frame,
            text=f"{stats['count']} photos",
            font=('Arial', 13),
            bg='#34495e',
            fg='#3498db'
        ).pack(pady=8)
        ...
        # Créer l'archive
        zip_path = self.gallery_downloader.create_zip_archive()
        
        progress_win.destroy()
        
        if zip_path and zip_path.exists():
            # ✅ NOUVEAU : Afficher les options avec QR code
            self.show_zip_download_options(zip_path, stats)
        else:
            messagebox.showerror("Erreur", "Impossible de créer l'archive")


    def show_zip_download_options(self, zip_path, stats):
        ...
        # Centrer (AVANT grab_set)
        options_win.update_idletasks()
        x = (options_win.winfo_screenwidth() // 2) - 300
        y = (options_win.winfo_screenheight() // 2) - 250
        options_win.geometry(f"600x500+{x}+{y}")
        
        # grab_set APRÈS que la fenêtre soit visible
        ...
        # Afficher le temps d'expiration
        expiration = self.gallery_downloader.get_zip_expiration_time(zip_path)
        if not expiration['expired']:
            tk.Label(
                info_frame,
                text=f"⏱️ Expire dans {expiration['remaining_minutes']} minutes",
                font=('Arial', 10),
                bg='#34495e',
                fg='#e67e22'
            ).pack(pady=5, padx=10)
        ...
        def show_qr():
            ...
            if not qr_plugin or not qr_plugin.is_initialized():
                messagebox.showerror("Erreur", "Plugin QR Code non disponible\nInstallez: pip install qrcode[pil]", parent=options_win)
                return
            
            # Générer le QR code
            result = self.gallery_downloader.generate_download_qr(qr_plugin)
            if not result:
                messagebox.showerror("Erreur", "Impossible de générer le QR code", parent=options_win)
                return
            
            # result est un tuple (qr_path, download_url)
            qr_path, download_url = result
            
            # Vérifier que qr_path est une chaîne valide
            if not qr_path or not isinstance(qr_path, (str, Path)):
                messagebox.showerror("Erreur", "QR code invalide", parent=options_win)
                return
            
            # Convertir en Path et vérifier l'existence
            qr_path_obj = Path(str(qr_path))
        ...
        self.gallery_downloader.clean_old_exports(keep_last=3)
