...
class photovincAppComplete:
    ...
    def __init__(self, root):
        ...
        # Détection d'imprimante automatique
        ...
        logger.info(f"Détection imprimante: {printer_msg}")
    ...
    def initialize_plugins(self):
        ...
        # Afficher infos imprimante si disponible
        if self.printer_integration.selected_printer:
            printer_info = self.printer_integration.get_printer_info()
            print(f"✓ Imprimante: {printer_info['name']} ({printer_info['model']})")
        else:
            print("⚠ Aucune imprimante détectée - Mode démo activé")
    ...
    def update_printer_status_label(self):
        """Met à jour le label de statut imprimante"""
            ...
            if messagebox.askyesno("Confirmer", f"Imprimer toutes les {len(self.last_session_photos)} photos ?", parent=selection_win):
                ...
                if printer and printer.is_initialized():
                    ...
                    for photo in self.last_session_photos:
                        if printer.print_image(str(photo)):
                            self.print_counter.increment_print()
                            success_count += 1
                        time.sleep(2)
                    self.update_counter_display()
